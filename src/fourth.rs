...
pub fn do_sort<T, F>(x: &mut [T],up: bool, comparator: &F) 
where T: Send,
      F: Sync + Fn(&T, &T) -> Ordering
{
    if x.len() > 1 {
        let mid_point = x.len() / 2;
        let (head, last) = x.split_at_mut(mid_point);

        if(mid_point >= PARALLEL_THRESHOLD) {
            rayon::join(|| do_sort(head, true, comparator),
                        || do_sort(last, false, comparator));
        } else {
            do_sort(head, true, comparator);
            do_sort(last, false, comparator);
        }
...
{
    if x.len() > 1 {
        compre_and_swap(x, forward,comparator);
        let mid_point = x.len() / 2;
        let (head, last) = x.split_at_mut(mid_point);

        if(mid_point >= PARALLEL_THRESHOLD) {
            rayon::join(|| sub_sort(head, forward, comparator),
                        || sub_sort(last, forward, comparator));
        } else {
            sub_sort(head, forward, comparator);
            sub_sort(last, forward, comparator);
        }
    }
}

fn compre_and_swap<T, F>(x: &mut [T], forward: bool, comparator: &F)
...
mod tests {
    ...
    fn sort_str_ascending() {
        ...
        assert_eq!(x, vec!["GC", "Rust", "and", "fast", "is", "memory","no",  "with"]);
    }
    #[test]
    fn sort_to_fail() {
